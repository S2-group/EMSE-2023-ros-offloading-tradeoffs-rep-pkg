---
title: "Effect of the number of SLAM filter particles parameter on the average feature extraction time"
output:
  html_notebook: default
---

We need to analyse the impact of two possible values for the number of SLAM filter particles on the average feature extraction time. We have chosen two values for the number of particles, namely, `5`, which is used in the primary experiments, and `30`, which is the default value in Gmapping ROS package. First, let us load the necessary libraries:
```{r}
...
data <- subset(data, select = c(X__run_id, particles, avg_extraction_time_ms))
data$particles <- as.factor(data$particles)
data$avg_extraction_time_ms <- as.numeric(data$avg_extraction_time_ms)
```

Mean and standard deviation of the average feature extraction time for both particle number values are displayed in the summary below:
```{r}
data %>%
  group_by(particles) %>%
...
```{r}
ggplot(data, aes(x = particles, y = avg_extraction_time_ms, fill = particles)) +
  theme_bw() + 
  geom_boxplot(width = 0.5, outlier.size = 0.5) + 
  geom_jitter(width = 0.2) +
  stat_summary(fun = mean, color = 'black', geom = 'point', shape = 5, size = 1.5) +
  labs(x = "Number of SLAM filter particles (#)", y = "Average feature extraction time (ms)") +
...
As the result above shows, there is only one outlier, for run 1, but it is not considered extreme in terms of statistical significance. We can proceed with other assumptions.
...
```{r}
ggqqplot(data, x = "avg_extraction_time_ms", facet.by = "particles")
```
...
```
As the results of the test show, p-values for both groups are higher than `0.05` reference value (`p = 0.067` and `p = 0.369`, respectfully), which means that we cannot reject the null-hypothesis stating that the data within both groups are normally distributed. Hereby, we conclude that the normality assumption for both groups is satisfied.
...
```{r}
data %>%
  t_test(avg_extraction_time_ms ~ particles) %>%
  add_significance()
```
The resulting p-value of $0.673$ is higher than `0.05` reference value for the `0.95` confidence interval. This means that the obtained p-value is not statistically significant and we cannot reject $H_0$, which states that the means of the average feature extraction time for both particle number values are equal. We can hereby conclude that the number of particles parameter does not have a statistically significant effect on the average feature extraction time.
