---
title: "Effect of the number of SLAM filter particles parameter on the total size of network packets"
output:
  html_notebook: default
---

We need to analyse the impact of two possible values for the number of SLAM filter particles on the total size of network packets. We have chosen two values for the number of particles, namely, `5`, which is used in the primary experiments, and `30`, which is the default value in Gmapping ROS package. First, let us load the necessary libraries:
```{r}
...
data <- subset(data, select = c(X__run_id, particles, size_of_packets))
data$particles <- as.factor(data$particles)
data$size_of_packets <- as.numeric(data$size_of_packets)
```

Mean and standard deviation of the total size of network packets for both particle number values are displayed in the summary below:
```{r}
data %>%
  group_by(particles) %>%
...
```{r}
ggplot(data, aes(x = particles, y = size_of_packets, fill = particles)) +
  theme_bw() + 
  geom_boxplot(width = 0.9, outlier.size = 0.5) + 
  stat_summary(fun = mean, color = 'black', geom = 'point', shape = 5, size = 1.5) +
  labs(x = "Number of SLAM filter particles (#)", y = "Total size of packets (MB)") +
...
```{r}
ggqqplot(data, x = "size_of_packets", facet.by = "particles")
```
...
```
As the results of the test show, p-values for both groups are higher than `0.05` reference value (`p = 0.340` and `p = 0.468`, respectfully), which means that we cannot reject the null-hypothesis stating that the data within both groups are normally distributed. Hereby, we conclude that the normality assumption for both groups is satisfied.
...
```{r}
data %>%
  t_test(size_of_packets ~ particles) %>%
  add_significance()
```
The resulting p-value of $0.94$ is much higher than `0.05` reference value for the `0.95` confidence interval. This means that the obtained p-value is not statistically significant and we cannot reject $H_0$, which states that the means of the total size of network packets for both particle number values are equal. We can hereby conclude that the number of filter particles parameter does not have a statistically significant effect on the total size of network packets.
