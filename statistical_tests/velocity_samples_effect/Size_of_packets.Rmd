...
Mean and standard deviation of the total size of network packets for both number of velocity samples values are displayed in the summary below:
```{r}
data %>%
...
ggplot(data, aes(x = velocity_samples, y = size_of_packets, fill = velocity_samples)) +
  ...
  labs(x = "Number of translation and rotation velocity samples (#)", y = "Total size of packets (B)") +
...
```
As the results of the test show, p-values for both groups are higher than `0.05` reference value (`p = 0.866` and `p = 0.823`, respectfully), which means that we cannot reject the null-hypothesis stating that the data within both groups are normally distributed. Hereby, we conclude that the normality assumption for both groups is satisfied.
...
data %>%
  ...
  add_significance()
```
The resulting p-value of $0.504$ is higher than `0.05` reference value for the `0.95` confidence interval. This means that the obtained p-value is not statistically significant and we cannot reject $H_0$, which states that the means of the total size of network packets for both number of velocity samples values are equal. We can hereby conclude that the number of velocity samples parameter does not have a statistically significant effect on the total size of network packets.
