...
Mean and standard deviation of the average navigation time for both number of velocity samples values are displayed in the summary below:
```{r}
data %>%
...
ggplot(data, aes(x = velocity_samples, y = avg_navigation_time_ms, fill = velocity_samples)) +
  ...
  labs(x = "Number of translation and rotation velocity samples (#)", y = "Average navigation time (ms)") +
...
As the result above shows, there are two outliers, for runs 8 and 9, respectfully, but neither is considered extreme in terms of statistical significance. We can proceed with other assumptions.
...
```
As the results of the test show, p-values for both groups are higher than `0.05` reference value (`p = 0.069` and `p = 0.797`, respectfully), which means that we cannot reject the null-hypothesis stating that the data within both groups are normally distributed. Hereby, we conclude that the normality assumption for both groups is satisfied.
...
data %>%
  ...
  add_significance()
```
The resulting p-value of `0.325` is higher than `0.05` reference value for the `0.95` confidence interval. This means that the obtained p-value is not statistically significant and we cannot reject $H_0$, which states that the means of the average navigation time for both number of velocity samples values are equal. We can hereby conclude that the number of velocity samples parameter does not have a statistically significant effect on the average navigation time.
