...
Mean and standard deviation of the average detection result delay for both velocity samples values are displayed in the summary below:
```{r}
data %>%
...
```
As the results of the test show, p-values for both groups are higher than `0.05` reference value (`p = 0.992` and `p = 0.938`, respectfully), which means that we cannot reject the null-hypothesis stating that the data within both groups are normally distributed. Hereby, we conclude that the normality assumption for both groups is satisfied.
...
data %>%
  ...
  add_significance()
```
The resulting p-value of $0.31$ is higher than `0.05` reference value for the `0.95` confidence interval. This means that the obtained p-value is not statistically significant and we cannot reject $H_0$, which states that the means of the average detection result delay for both number of velocity samples values are equal. We can hereby conclude that the number of velocity samples parameter does not have a statistically significant effect on the average detection result delay.
