...
data <- subset(data, select = c(X__run_id, frame_rate, avg_navigation_time))
...
```{r}
ggplot(data, aes(x = frame_rate, y = avg_navigation_time, fill = frame_rate)) +
  theme_bw() + 
  geom_boxplot(width = 0.9, outlier.size = 0.5) + 
  stat_summary(fun = mean, color = 'black', geom = 'point', shape = 5, size = 1.5) +
  labs(x = "Frame rate (fps)", y = "Average navigation time (s)") +
...
```{r}
data %>%
  rstatix::wilcox_test(avg_navigation_time ~ frame_rate) %>%
  add_significance()
```
