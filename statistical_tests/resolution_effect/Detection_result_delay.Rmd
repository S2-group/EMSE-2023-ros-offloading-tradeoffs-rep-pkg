---
title: "Effect of camera resolution parameter on the average detection result delay"
output:
  html_notebook: default
---

We need to analyse the impact of two possible values of camera resolution on the average detection result delay. We have chosen two values for resolution, namely, `640x480px`, which is used in the primary experiments, and two times higher resolution, `1280x960px`. First, let us load the necessary libraries:
```{r}
...
Mean and standard deviation of the average detection result delay for both resolutions are displayed in the summary below:
```{r}
data %>%
  group_by(resolution) %>%
...
```
Data points on the right figure, in `640x480px` resolution group, do seem to be aligned with the reference line. However, we cannot confirm this with great certainty for the `1280x960px` resolution group, since the data points on the left figure above seem to be scattered to an extent. This is why we need to perform Shapiro-Wilk test for both groups to complement QQ-plot conclusions:
```{r}
data %>%
  group_by(resolution) %>%
  shapiro_test(avg_detection_result_delay_ms)
```
As the results of the test show, p-values for both groups are higher than `0.05` reference value (`p = 0.973` and `p = 0.749`, respectfully), which means that we cannot reject the null-hypothesis stating that the data within both groups are normally distributed. Hereby, we conclude that the normality assumption for both groups is satisfied.
...
data %>%
  ...
  add_significance()
```
The resulting p-value of $2.42 \cdot 10^{-9}$ is lower than `0.05` reference value for the `0.95` confidence interval. This means that the obtained p-value is statistically significant and we can reject $H_0$, which states that the means of the average detection result delay for both resolution values are equal. We can hereby conclude that camera resolution parameter has a statistically significant effect on the average detection result delay.

## Effect size

We established with Welch's t-test result that camera resolution parameter has a statistically significant effect on the average detection result delay. Now we need to perform Cohen's d test to estimate the magnitude of the said effect:

```{r}
data %>%
  cohens_d(avg_detection_result_delay_ms ~ resolution)
```
The resulting effect size of `d = 10.00303` is interpreted as _large_ in R, according to the Cohen's rule of thumb^[http://www.utstat.toronto.edu/~brunner/oldclass/378f16/readings/CohenPower.pdf]. According to the new and extended rules of thumb defined by Sawilowsky, the magnitude for this d-value is considered as _huge_^[https://digitalcommons.wayne.edu/cgi/viewcontent.cgi?article=1536&context=jmasm]. This is indeed clear from the distribution of data for both treatments in the boxplot above, where we can see that the average detection result delay mean for `640x480px` resolution is `3.254 ms`, while it is `37.189 ms` for `1280x960px`. 
