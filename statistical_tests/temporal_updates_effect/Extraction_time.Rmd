...
data <- subset(data, select = c(X__run_id, temporal_updates, avg_extraction_time_ms))
data$temporal_updates <- as.factor(data$temporal_updates)
data$avg_extraction_time_ms <- as.numeric(data$avg_extraction_time_ms)
```

Mean and standard deviation of the average feature extraction time for both temporal updates values are displayed in the summary below:
```{r}
data %>%
...
```{r}
ggplot(data, aes(x = temporal_updates, y = avg_extraction_time_ms, fill = temporal_updates)) +
  theme_bw() + 
  geom_boxplot(width = 0.5, outlier.size = 0.5) + 
  geom_jitter(width = 0.2) +
  stat_summary(fun = mean, color = 'black', geom = 'point', shape = 5, size = 1.5) +
  labs(x = "Temporal updates", y = "Average feature extraction time (ms)") +
...
```{r}
ggqqplot(data, x = "avg_extraction_time_ms", facet.by = "temporal_updates")
```
...
```
As the results of the test show, p-value for temporal updates `on` group is higher than `0.05` reference value (`p = 0.813`), but lower for temporal updates `off` group. This means that the null hypothesis of data within temporal updates `off` group being normally distributed is rejected, meaning that the normality of the data assumption is not fulfilled. We will proceed with a non-parametric alternative to the independent two samples t-test, which is Wilcoxon rank sum test.
...
```{r}
data %>%
  rstatix::wilcox_test(avg_extraction_time_ms ~ temporal_updates) %>%
  add_significance()
```
The resulting p-value of $0.353}$ is higher than `0.05` reference value for the `0.95` confidence interval. This means that the obtained p-value is not statistically significant and we cannot reject $H_0$, which states that the means of the average feature extraction time for both temporal updates values are equal. We can hereby conclude that the temporal updates parameter does not have a statistically significant effect on the average feature extraction time.
