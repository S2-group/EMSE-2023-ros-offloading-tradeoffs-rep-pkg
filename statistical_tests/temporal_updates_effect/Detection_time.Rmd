...
data <- subset(data, select = c(X__run_id, temporal_updates, avg_detection_time_ms))
data$temporal_updates <- as.factor(data$temporal_updates)
data$avg_detection_time_ms <- as.numeric(data$avg_detection_time_ms)
```

Mean and standard deviation of the average object detection time for both temporal updates values are displayed in the summary below:
```{r}
data %>%
...
```{r}
ggplot(data, aes(x = temporal_updates, y = avg_detection_time_ms, fill = temporal_updates)) +
  theme_bw() + 
  geom_boxplot(width = 0.5, outlier.size = 0.5) + 
  geom_jitter(width = 0.2) +
  stat_summary(fun = mean, color = 'black', geom = 'point', shape = 5, size = 1.5) +
  labs(x = "Temporal updates", y = "Average object detection time (ms)") +
...
As the result above shows, there are two outliers, for runs 14 and 20, respectfully, but neither of them is considered extreme in terms of statistical significance. We can proceed with other assumptions.
...
```{r}
ggqqplot(data, x = "avg_detection_time_ms", facet.by = "temporal_updates")
```
...
```
As the results of the test show, p-values for both groups are higher than `0.05` reference value (`p = 0.230` and `p = 0.523`, respectfully), which means that we cannot reject the null-hypothesis stating that the data within both groups are normally distributed. Hereby, we conclude that the normality assumption for both groups is satisfied.
...
```{r}
data %>%
  t_test(avg_detection_time_ms ~ temporal_updates) %>%
  add_significance()
```
The resulting p-value of $0.432$ is higher than `0.05` reference value for the `0.95` confidence interval. This means that the obtained p-value is not statistically significant and we cannot reject $H_0$, which states that the means of the average object detection time for both temporal updates values are equal. We can hereby conclude that the temporal updates parameter does not have a statistically significant effect on the average object detection time.
