---
title: "Effect of the simulation time parameter on the total number of network packets"
output:
  html_notebook: default
---

We need to analyse the impact of two possible values for the simulation time on the total number of network packets. We have chosen two values for the simulation time, namely, `1.5 s`, which is used in the primary experiments, and two times higher value, `3 s`. First, let us load the necessary libraries:
```{r}
...
data <- subset(data, select = c(X__run_id, sim_period, num_of_packets))
data$sim_period <- as.factor(data$sim_period)
data$num_of_packets <- as.numeric(data$num_of_packets)
```

Mean and standard deviation of the total number of network packets for both simulation time values are displayed in the summary below:
```{r}
data %>%
  group_by(sim_period) %>%
...
As the result above shows, there is only one outlier, for run 5, but it is not considered extreme in terms of statistical significance. We can proceed with other assumptions.
...
data %>%
  ...
  add_significance()
```
TThe resulting p-value of $2.76 \cdot 10^{-5}$ is lower than `0.05` reference value for the `0.95` confidence interval. This means that the obtained p-value is statistically significant and we can reject $H_0$, which states that the means of the total number of network packets for both simulation time values are equal. We can hereby conclude that the simulation time parameter has a statistically significant effect on the total number of network packets.

## Effect size

We established with Welch's t-test results that the simulation time parameter has a statistically significant effect on the total number of network packets. Now we need to perform Cohen's d test to estimate the magnitude of the said effect:

```{r}
data %>%
  cohens_d(num_of_packets ~ sim_period)
```
The resulting effect size of `d = -2.527146` is interpreted as _large_ in R, according to the Cohen's rule of thumb^[http://www.utstat.toronto.edu/~brunner/oldclass/378f16/readings/CohenPower.pdf]. According to the new and extended rules of thumb defined by Sawilowsky, the magnitude for this d-value is considered as _huge_^[https://digitalcommons.wayne.edu/cgi/viewcontent.cgi?article=1536&context=jmasm]. This is indeed clear from the distribution of data for both treatments in the boxplot above, where we can see that the total number of network packets mean for `1.5 s` simulation time is `44863.6` packets, while it is `56137.1` packets for `3 s` simulation time. The resulting d-value is negative since the total number of network packets mean for `1.5 s` simulation time group is lower than the one for `3 s` simulation time.
