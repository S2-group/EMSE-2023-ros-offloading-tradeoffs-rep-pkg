---
title: "Effect of the simulation time parameter on the average feature extraction time"
output:
  html_notebook: default
---

We need to analyse the impact of two possible values for the simulation time on the average feature extraction time. We have chosen two values for the simulation time, namely, `1.5 s`, which is used in the primary experiments, and two times higher value, `3 s`. First, let us load the necessary libraries:
```{r}
...
data <- subset(data, select = c(X__run_id, sim_period, avg_extraction_time_ms))
data$sim_period <- as.factor(data$sim_period)
data$avg_extraction_time_ms <- as.numeric(data$avg_extraction_time_ms)
```

Mean and standard deviation of the average feature extraction time for simulation time values are displayed in the summary below:
```{r}
data %>%
  group_by(sim_period) %>%
...
```{r}
ggplot(data, aes(x = sim_period, y = avg_extraction_time_ms, fill = sim_period)) +
  theme_bw() + 
  geom_boxplot(width = 0.5, outlier.size = 0.5) + 
  geom_jitter(width = 0.2) +
  stat_summary(fun = mean, color = 'black', geom = 'point', shape = 5, size = 1.5) +
  labs(x = "Simulation time (s)", y = "Average feature extraction time (ms)") +
...
```{r}
ggqqplot(data, x = "avg_extraction_time_ms", facet.by = "sim_period")
```
...
```
As the results of the test show, p-values for both groups are higher than `0.05` reference value (`p = 0.890` and `p = 0.131`, respectfully), which means that we cannot reject the null-hypothesis stating that the data within both groups are normally distributed. Hereby, we conclude that the normality assumption for both groups is satisfied.
...
```{r}
data %>%
  t_test(avg_extraction_time_ms ~ sim_period) %>%
  add_significance()
```
The resulting p-value of $0.575$ is higher than `0.05` reference value for the `0.95` confidence interval. This means that the obtained p-value is not statistically significant and we cannot reject $H_0$, which states that the means of the average feature extraction time for both simulation time values are equal. We can hereby conclude that the simulation time parameter does not have a statistically significant effect on the average feature extraction time.
